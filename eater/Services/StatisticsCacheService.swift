...
class StatisticsCacheService {
    ...
    func getCachedStatistics(for dateString: String) -> DailyStatistics? {
        guard let cachedData = getCachedData(),
              ...
              !isExpired(dateString: dateString, cachedTime: statistics.cachedTime) else {
            return nil
        }
    ...
    func getCachedStatistics(for dateStrings: [String]) -> [DailyStatistics] {
        ...
        return dateStrings.compactMap { dateString in
            guard let cachedEntry = cachedData[dateString],
                  !isExpired(dateString: dateString, cachedTime: cachedEntry.cachedTime) else {
                return nil
            }
...
extension DailyStatistics: Codable {
    ...
    init(from decoder: Decoder) throws {
        ...
        self.init(
            ...
            totalFoodWeight: try container.decode(Int.self, forKey: .totalFoodWeight),
            personWeight: try container.decode(Float.self, forKey: .personWeight),
            ...
            hasData: try container.decodeIfPresent(Bool.self, forKey: .hasData) ?? true // Default to true for backward compatibility
        )
    }
