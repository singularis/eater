...
final class Localization {
    ...
    private func loadTranslations(code: String) -> [String: String] {
        ...
        if let url = Bundle.main.url(forResource: code, withExtension: "json", subdirectory: "Localization"),
        ...
        if let urls = Bundle.main.urls(forResourcesWithExtension: "json", subdirectory: nil) {
            if let match = urls.first(where: { $0.lastPathComponent.lowercased() == "\(code.lowercased()).json" }),
