...
final class ShareFoodViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int { emails.count }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        promptSharePercentage { [weak self] percentage in
            guard let self = self else { return }
            GRPCService().shareFood(time: self.time, fromEmail: fromEmail, toEmail: toEmail, percentage: percentage) { [weak self] success in
                DispatchQueue.main.async {
                    guard let self = self else { return }
                    if success {
    ...
    private func promptSharePercentage(onSelected: @escaping (Int32) -> Void) {
        let alert = UIAlertController(title: "How much did your friend eat?", message: nil, preferredStyle: .actionSheet)
        let options: [Int32] = [25, 50, 75]
        for v in options {
            alert.addAction(UIAlertAction(title: "\(v)%", style: .default, handler: { _ in onSelected(v) }))
        }
    ...
    private func promptCustomPercentage(onSelected: @escaping (Int32) -> Void) {
        let alert = UIAlertController(title: "Custom percentage", message: "Enter a value between 1 and 300", preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { _ in
            if let text = alert.textFields?.first?.text, let value = Int(text), value > 0, value <= 300 {
