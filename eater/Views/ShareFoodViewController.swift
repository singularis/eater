...
final class ShareFoodViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
  ...
  // Backend returns the full friends list in one response. Fetch everyone, sort by
  // share count, then reveal pages locally so the first screen is the most-shared friends.
          ...
          if success {
            ...
            self.dismiss(animated: true) {
              ...
              AlertHelper.showAlert(
                title: loc("share.success.title", "Shared"), message: message)
            }
          } else {
            AlertHelper.showAlert(
  ...
  private func promptSharePercentage(onSelected: @escaping (Int32) -> Void) {
    ...
    // Green so "same amount" stands out from 25/50/75%.
